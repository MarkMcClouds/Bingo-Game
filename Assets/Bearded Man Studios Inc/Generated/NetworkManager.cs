...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ChattingManagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ChattingManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ChattingManagerNetworkObject.Length > 0 && ChattingManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ChattingManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateChattingManager instead, its shorter and easier to type out ;)")]
		public ChattingManagerBehavior InstantiateChattingManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChattingManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChattingManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ChattingManagerBehavior>().networkObject = (ChattingManagerNetworkObject)obj;
		...
		public ChattingManagerBehavior InstantiateChattingManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChattingManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChattingManagerBehavior>();
